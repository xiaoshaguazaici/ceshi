...
//2 busy done cs 信号的提前给出（运行要靠这几个逻辑）
        ...
        assign CS = (CS_PL)?(!ro_CS):(ro_CS);//CS_PL = 0 是以0为有效，1是以1有效
        ///done信号
        always @(posedge i_sys_clk , negedge i_reset_n) begin
                if(!i_reset_n)
                        spi_done <= 'd0;
...
//7 线性序列机 mosi 与miso
        ...
        always @(posedge i_sys_clk , negedge i_reset_n) begin
                if(!i_reset_n)
                        ...
                        //else if(EDGE == 'd2 && data_cnt == TX_DATA_WIDTH -1)//慢一个周期合适
                        else if(!spi_busy)
                                MOSI <= 'd0;//默认0 
                        else
                                MOSI <= MOSI;   
